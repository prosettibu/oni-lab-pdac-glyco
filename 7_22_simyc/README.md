...
- Salmon index + tx2gene mapping built from GENCODE v46 (GRCh38 primary
  ...
  env vars.
- `cptac_glyco/` (`PDAC_meta.xlsx`, `PDAC_nglycoform.tsv`) — a CPTAC PDAC
  glycoproteomics reference dataset, not generated by any script here.
  Obtain from the CPTAC data portal and add a specific accession/citation
...
| `02_salmon_quant_simyc.sh` | `fastq/` | `salmon_quant_simyc/` | Salmon quant, siMYC samples. **Reconstructed** — no original script was found for this step; verify against how the existing `salmon_quant_simyc/` output was actually produced. |
...
- `redownload_failed_erk.sh` (a one-off patch re-downloading 3 specific
  corrupted ERKi files) is intentionally not included here — it documents a
  download hiccup from data acquisition, not a general/reproducible pipeline
  step.
